...
def test_cmp_sets_cf_before_sbb():
    ...
    assert "_cf = ((uint32_t)(eax) < (uint32_t)(ebx))" in out, out
    assert "_cf ? 0xFFFFFFFF : 0" in out, out
...
def test_add_sets_cf_carry_into_adc():
    ...
    assert "_cf = ((uint32_t)(eax) < (uint32_t)(ecx))" in out, out
    assert "edx = edx + edx + _cf" in out, out
...
def test_xor_clears_cf():
    ...
    assert "_cf = 0; /* xor clears CF */" in out, out
    assert "_cf ? 0xFFFFFFFF : 0" in out, out
...
def test_neg_sets_cf():
    ...
    assert "neg: CF = (operand != 0)" in out, out
    assert "_cf ? 0xFFFFFFFF : 0" in out, out
